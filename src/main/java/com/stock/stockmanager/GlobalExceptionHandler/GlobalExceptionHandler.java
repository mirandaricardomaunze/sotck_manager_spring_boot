...
import com.stock.stockmanager.exception.IllegalStateException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<?> handleNotFound(ResourceNotFoundException ex) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body( Map.of(
                "error","Não encontrado","message", ex.getMessage()));
    }
    ...
    public ResponseEntity<?> handleDuplicate(DuplicateResourceException e) {
        return ResponseEntity.status(HttpStatus.CONFLICT).body(Map.of(
                "error","Recurso duplicado","message",e.getMessage()
        ));
    }
    ...
    public ResponseEntity<?> handleValidation(MethodArgumentNotValidException e) {
        ...
        return ResponseEntity.badRequest().body(Map.of(
                "error","Erro de validação","validationErros", errors));
    }
    ...
    public ResponseEntity<?> handleInternalError(Exception e) {
        ...
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                .body(Map.of("error", "Erro inesperado do servidor ",
                        "message",
    ...
    public ResponseEntity<?> handleJasperReportError(JRException e) {
        ...
        body.put("error", "Erro de compilaçaão de relatório ");
